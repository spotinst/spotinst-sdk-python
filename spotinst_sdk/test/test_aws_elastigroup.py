...
class AwsElastigroupTestCase(unittest.TestCase):
    ...
    def setUp(self):
        self.client = SpotinstClient(auth_token='dummy-token', account_id='dummy-account')
...
class AwsElastigroupTestMlbRuntimeIntegration(AwsElastigroupTestCase):
    def runTest(self):
        ...
        third_party_integrations = ThirdPartyIntegrations(mlb_runtime=mlb_runtime)
...
class AwsElastigroupTestSchedulingIntegration(AwsElastigroupTestCase):
    def runTest(self):
        scheduled_task1 = ScheduledTask(frequency='hourly', task_type='backup_ami')
        scheduled_task2 = ScheduledTask(cron_expression='00 17 * * 3', task_type='roll', batch_size_percentage=30)
        scheduled_task3 = ScheduledTask(cron_expression='00 22 * * 3', task_type='scale',
                                        start_time='2018-05-23T10:55:09Z', scale_target_capacity=0,
                                        scale_min_capacity=0,
                                        scale_max_capacity=3)
        scheduled_task4 = ScheduledTask(task_type='scale', start_time='2018-05-25T10:55:09Z', scale_target_capacity=3,
                                        scale_min_capacity=3,
                                        scale_max_capacity=3)

        scheduling = Scheduling(tasks=[scheduled_task1, scheduled_task2, scheduled_task3, scheduled_task4])
